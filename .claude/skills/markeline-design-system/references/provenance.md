...
- 2026-09-01 evals iteration-1（LP / スライド3枚 / リッチメニュー、各スキルあり・なし）: スキルあり 94%、なし 57%。なしは3本とも LINE 公式の緑 `#06C755` をブランド色として採用し、スライドでは未確認の成果数値を実績として記載。ありは全件 lint 0 件、数値は「例」明示。唯一の不合格は LP の番号バッジで `brand-teal` を 16px 文字に使った件（SKILL.md に太字 19px 規則を追記して対処）。
