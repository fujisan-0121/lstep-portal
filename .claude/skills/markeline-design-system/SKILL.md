...
- スライドは常に「1 枚 1 メッセージ、文字は大きく（24px 以上）」。会議室の後ろから読めない資料は失敗。要素が入らないなら枚数を増やす。
- スライドの文字サイズは `references/reference-type-scale.md`（基準資料 14 枚を 1 行ずつ実測した値）から取る。目分量で決めない。テンプレートの CSS は既にその値。
...
- [ ] （インフォグラフィック型）全枚にタイトル帯とロゴボックス、結論帯かページ番号がある。文字が 24px 未満になっていない（注記・ページ番号は 22px まで）
- [ ] （スライド全般）1 枚 1 メッセージ。タイトル帯が主張文になっていて、根拠ブロックは 4 個以下、箇条書きは 1 項目 1 行。入らないなら枚数を増やす
- [ ] mono の中に和文が入っていない
