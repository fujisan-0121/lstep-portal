...
| クライアント提案書 / セミナー投影資料 / 営業資料 / インフォグラフィック（**標準**） | `references/infographic.md` | `templates/infographic.html` |
| 社内資料 / 報告書 / 読み物スライド（余白多め・編集誌風） | `references/slides.md` | `templates/slide.html` |
...
**スタイルは 2 つ。先に決める**
- **インフォグラフィック型**（提案書・セミナー・営業資料の標準。基準は「野村工務店様 ご提案」）: ティールのタイトル帯、番号付きカード、巨大数字、黄色の結論帯、単色ピクトグラム。情報密度高め。詳細は `references/infographic.md`。
- **編集誌型**（社内資料・LP・報告書）: 余白多め、黄色は最小限。詳細は `references/slides.md` / `references/lp.md`。
- 「提案書」「セミナー」「プレゼン」「営業資料」と言われたら、指定がない限りインフォグラフィック型。

**色**
- ティール = 主役・面・信頼。イエロー = 行動と結論。
  編集誌型では 1 成果物につき「最重要の一点」だけ（CTA ボタン、結論の下線、KPI の一つ）。面積が画面の 5% を超えたら安売り感が出る。
  インフォグラフィック型では 1 枚に「結論帯 + 強調 1〜2 語 + 矢印」まで使ってよい。面積 15% まで。黄色の面を 2 つ隣接させない。
...
**フォント**
- 和文と本文は `Noto Sans JP`（基準資料「野村工務店様 ご提案」と同じ源ノ角ゴシック系）。見出し 900、本文 400/500、強調 700。他の Google 日本語フォントに勝手に変えない。
- 数値・KPI・日付・ページ番号・ラベルキャップスは `IBM Plex Mono`。**mono の中に漢字かなを入れない**（システムの等幅にフォールバックして崩れる）。
  インフォグラフィック型の巨大数字（96px 以上）だけは `Noto Sans JP`（display = sans と同一） 700（`--ml-font-display`）。投影で等幅は硬く見えるため。
...
- フォントは `assets/fonts/`（Noto Sans JP 可変 400〜900、IBM Plex Mono）に同梱済みで、`tokens.css` が `@import` する。ネットワークに依存しないので、どの環境でも同じ書体で描画される。
  成果物を別フォルダに置くときは `assets/fonts/` も一緒にコピーし、`tokens.css` の `@import` パスを合わせる。`document.fonts` に "Noto Sans JP 400 900" が出ていれば読めている。
- 納品は最高画質で: `render_preview.py --scale 2`（3840×2160 PNG）と `--pdf`（ベクター PDF、文字埋め込み）。JPEG 圧縮や画像貼り込み pptx にしない。
...
- [ ] イエローの使用が、編集誌型なら 1 役割、インフォグラフィック型なら「結論帯 + 強調 + 矢印」の範囲に収まっている
- [ ] （インフォグラフィック型）全枚にタイトル帯とロゴボックス、結論帯かページ番号がある。文字が 18px 未満になっていない
- [ ] mono の中に和文が入っていない
