...
**スタイルは 2 つ。先に決める**
- **インフォグラフィック型**（提案書・セミナー・営業資料の標準。基準は基準デッキ（工務店向け提案））: ティールのタイトル帯、番号付きカード、巨大数字、黄色の結論帯、単色ピクトグラム。情報密度高め。詳細は `references/infographic.md`。
...
**フォント**
- 和文と本文は `Noto Sans JP`（基準デッキ（工務店向け提案）と同じ源ノ角ゴシック系）。見出し 900、本文 400/500、強調 700。他の Google 日本語フォントに勝手に変えない。
